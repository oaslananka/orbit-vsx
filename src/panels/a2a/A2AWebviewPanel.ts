...
function renderA2ADetailHtml(card: AgentCard, _scriptUri: vscode.Uri, nonce: string): string {
  const skillsHtml = card.skills
    .map(
      (s) =>
        `<div class="skill-item"><strong>${escapeHtml(s.name)}</strong><br><span style="opacity:0.7;">${escapeHtml(s.description)}</span></div>`
    )
    .join('');
